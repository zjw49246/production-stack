...
def test_route_request_with_session_id():
    ...
    request_stats = {
        "http://engine1.com": RequestStats(qps=10), "http://engine2.com": RequestStats(qps=5)}
    ...
    assert url == router.route_request(
...
def test_consistent_hashing_remove_node_multiple_sessions():
    ...
    urls_before = [router.route_request(
        endpoints, None, request_stats, req) for req in requests]
    ...
    urls_after = [router.route_request(
        endpoints, None, request_stats, req) for req in requests]

    # Ensure all session IDs are still mapped to valid endpoints
    assert all(url in [endpoint.url for endpoint in endpoints]
...
def test_consistent_hashing_add_then_remove_node():
    ...
    # Route with initial endpoints
    urls_before_add = [router.route_request(
        endpoints, None, request_stats, req) for req in requests]
    ...
    urls_after_add = [router.route_request(
        endpoints, None, request_stats, req) for req in requests]

    # Ensure all session IDs are still mapped to valid endpoints
    assert all(url in [endpoint.url for endpoint in endpoints]
               for url in urls_after_add)
    ...
    urls_after_remove = [router.route_request(
        endpoints, None, request_stats, req) for req in requests]

    # Ensure all session IDs are still mapped to valid endpoints
    assert all(url in [endpoint.url for endpoint in endpoints]
               for url in urls_after_remove)
