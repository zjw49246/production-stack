...
def response_consumer(response_stream, start_time):
    ...
    try:
        ...
        print(f"Error in consumer thread {threading.current_thread().name} of process {os.getpid()}: {e}")
    final_words = "".join(chunk_messages)
    end_time = time.time()
    response_len = len(final_words.split(' '))
    throughput = response_len / (end_time - start_time)
    print(f"Process {os.getpid()} got a response of: {response_len} words in {end_time-start_time:.2f} seconds (throughput: {throughput:.2f} w/s, ttft: {token_time - start_time:.4f}) at {end_time}")
...
def worker(api_key, base_url, model, qps_per_worker):
    ...
    while True:
        ...
        try:
            ...
            print("Process {} sent a request at {:.4f}, connection overhead: {:.4f}".format(os.getpid(), start_time, start_time - start))

            consumer_thread = threading.Thread(target=response_consumer, args=(response_stream, start_time), daemon=True)
...
def main():
    ...
    parser.add_argument("--qps", type=float, required=True, help="Total queries per second")
    parser.add_argument("--num-workers", type=int, required=True, help="Number of worker processes")
    ...
    for _ in range(args.num_workers):
        p = multiprocessing.Process(target=worker, args=(api_key, base_url, model, qps_per_worker))
        p.start()
        processes.append(p)
